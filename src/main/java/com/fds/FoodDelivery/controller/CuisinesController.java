...
public static String TAG = "[Cuisines]";
	...
	public Response<Cuisines> getCuisineById(@PathVariable("id") Integer id) {
		...
		if (cuisined.getId() == id) {
			return new Response<Cuisines> (101, cuisined+" "+TAG+"s Fetched Successful on "+date,cuisined);
		} else {
			return new Response<Cuisines> (101, cuisined+" "+TAG+"s Fetched not Found  on "+date,cuisined);
		}
	}
